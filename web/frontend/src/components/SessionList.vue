<template>
  <div class="session-list">
    ...
    <!-- 格式 Tab（多格式时显示） -->
    <div v-if="settingsStore.reasonixEnabled || settingsStore.claudeCodeEnabled || settingsStore.opencodeEnabled" class="format-tabs">
      <button
        v-if="settingsStore.reasonixEnabled"
        class="format-tab"
        :class="{ active: sessionStore.activeTab === 'reasonix' }"
        @click="sessionStore.setActiveTab('reasonix')"
      >
        {{ $t('session.format_reasonix') }}
        <span class="tab-count">{{ sessionStore.reasonixSessions.length }}</span>
      </button>
      <button
        class="format-tab"
...
// 关闭 Reasonix 支持时，强制切回 Codex
watch(() => settingsStore.reasonixEnabled, (val) => {
  if (!val && sessionStore.activeTab === 'reasonix') {
    sessionStore.setActiveTab('codex')
  }
})
...
const visibleSessions = computed(() => {
  ...
  if (!settingsStore.reasonixEnabled && sessionStore.activeTab === 'reasonix') {
    return sessionStore.codexSessions
  }
