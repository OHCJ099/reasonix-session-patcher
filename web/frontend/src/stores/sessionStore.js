...
export const useSessionStore = defineStore('session', () => {
  ...
  const activeTab = ref('reasonix') // 'reasonix' | 'codex' | 'claude_code' | 'opencode'
  ...
  const reasonixSessions = computed(() => sessions.value.filter(s => s.format === 'reasonix'))

  // 当前 Tab 的会话
  const activeTabSessions = computed(() => {
    if (activeTab.value === 'reasonix') return reasonixSessions.value
  ...
  async function fetchSessions(checkRefusal = true) {
    ...
    try {
      ...
      if (!_tabInitialized) {
        ...
        // Reasonix Desktop fork：本机会话优先
        if (reasonixSessions.value.length > 0) {
          activeTab.value = 'reasonix'
