...
DEFAULT_REASONIX_SESSION_DIR = os.path.join(os.environ.get("APPDATA", expand_user_path("~")), "reasonix", "sessions")
...
def _to_schema_format(fmt: SessionFormat) -> SessionFormatEnum:
    ...
    if fmt == SessionFormat.CLAUDE_CODE:
        ...
        return SessionFormatEnum.REASONIX
...
def list_sessions(
    ...
    if session_format is None:
        ...
        if os.path.exists(DEFAULT_REASONIX_SESSION_DIR):
            scan_targets.append((DEFAULT_REASONIX_SESSION_DIR, SessionFormat.REASONIX))
        ...
        scan_targets.append((DEFAULT_CLAUDE_SESSION_DIR, SessionFormat.CLAUDE_CODE))
    elif session_format == SessionFormat.REASONIX:
        scan_targets.append((DEFAULT_REASONIX_SESSION_DIR, SessionFormat.REASONIX))
...
def _session_core_format(session: Session) -> SessionFormat:
    ...
    if session.format == SessionFormatEnum.CLAUDE_CODE:
        ...
        return SessionFormat.OPENCODE
    elif session.format == SessionFormatEnum.REASONIX:
        return SessionFormat.REASONIX
    return SessionFormat.CODEX
...
def preview_session(file_path: str, mock_response: str = MOCK_RESPONSE,
    ...
    for idx, line in enumerate(parsed_lines):
        ...
        # Reasonix Desktop：顶层 role/content 消息格式
        if (session_format == SessionFormat.REASONIX or (line.get('role') in ('user', 'assistant', 'system', 'tool') and 'content' in line)):
            msg_role = line.get('role')
