    ...
    if fmt == 'codex':
        ...
        if args.session_dir is not None and args.session_dir != argparse.SUPPRESS:
            ...
            if expanded.startswith(opencode_dir):
                return SessionFormat.OPENCODE
            if expanded.startswith(reasonix_dir):
                return SessionFormat.REASONIX

        # 本 fork 面向 Reasonix Desktop：auto 优先选择本机会话，其次保持上游兼容。
        reasonix_dir = os.path.join(os.environ.get("APPDATA", os.path.expanduser("~")), "reasonix", "sessions")
        codex_dir = os.path.expanduser("~/.codex/sessions/")
        claude_dir = os.path.expanduser("~/.claude/projects/")
        if os.path.exists(reasonix_dir):
            return SessionFormat.REASONIX
...
def main():
    parser = argparse.ArgumentParser(
        description='Session Patcher - 清理 Reasonix Desktop / Codex CLI / Claude Code / OpenCode 会话中的拒绝回复和无效 thinking blocks'
    )

    # 会话清理参数
    parser.add_argument('--session-dir', default=None,
                        help='会话目录 (默认根据 --format 自动选择)')
    parser.add_argument('--format', choices=['reasonix', 'codex', 'claude-code', 'opencode', 'auto'],
                        default='auto',
    ...
    format_label = {
        ...
        SessionFormat.REASONIX: 'Reasonix Desktop',
